d3_workload = function() {
	...
	var drawGraph = function(data) {
			...
			y = y.domain([d3.max(data, function(d) {
				return d.LastResult / second
			}), 0]);
	...
	function drawCommandBlocks(bars) {
		bars.transition()
			...
			.attr("y", function(d) {
				return y((d.y + d.height) / second)
			})
			.attr("height", function(d) {
				return (svgHeight - y((d.height) / second))
			})
	...
	function drawIterationOverlay(iterations, len) {
		iterations.transition()
			...
			.attr("y", function(d) {
				return y(d.LastResult / second) + 3
			})
			.attr("height", function(d) {
				return (svgHeight - y(d.LastResult / second)) - 5
			});
	...
	for (var i = 0; i < commands.length; i++) {		
		for (var k in commands[i].Commands) {
			list.push({
				"label": k,				
				"y": 0,
				"height": commands[i].Commands[k].TotalTime,
				"iteration": i
			})
		}
